...
export const createScheduleAllDoctors = async (data) => {
    const config = {
        headers: {
            "Content-Type": "application/json",
        },
    ...
    const response = await axios.post(`${process.env.REACT_APP_API}/schedule/create/doctors`, data, config);
...
export const deleteScheduleByDoctorApi = async (scheduleId, doctorId) => {
    const config = {
        headers: {
            "Content-Type": "application/json",
        },
    };
    const response = await axios.delete(`${process.env.REACT_APP_API}/schedule/${scheduleId}/${doctorId}`);
